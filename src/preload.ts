...
console.log('[Denuel Chat Desktop] Preload.ts');
...
const start = async (): Promise<void> => {
  console.log('[Denuel Chat Desktop] Preload.ts start fired');
  const serverUrl = await invoke('server-view/get-url');

  if (retryCount > 5) return;

  if (!serverUrl) {
    console.log('[Denuel Chat Desktop] serverUrl is not defined');
    console.log('[Denuel Chat Desktop] Preload start - retrying in 1 seconds');
  ...
  console.log('[Denuel Chat Desktop] waiting for RocketChatDesktop.onReady');
  RocketChatDesktop.onReady(() => {
    console.log('[Denuel Chat Desktop] RocketChatDesktop.onReady fired');
    listen(
...
console.log('[Denuel Chat Desktop] waiting for window load');
