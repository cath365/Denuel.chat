...
console.log('[Denuel Chat Desktop] Injected.ts');
...
const resolveWithExponentialBackoff = <T>(
  ...
  new Promise<T>((resolve) => resolve(fn())).catch((error) => {
    ...
    console.log(
      '[Denuel Chat Desktop] Inject resolveWithExponentialBackoff - retrying in 1 seconds'
    );
...
const start = async () => {
  console.log('[Denuel Chat Desktop] Injected.ts start fired');
  if (typeof window.require !== 'function') {
    console.log('[Denuel Chat Desktop] window.require is not defined');
    console.log('[Denuel Chat Desktop] Inject start - retrying in 1 seconds');
  ...
  if (!serverInfo.version) {
    console.log('[Denuel Chat Desktop] serverInfo.version is not defined');
    return;
  }

  console.log('[Denuel Chat Desktop] Injected.ts serverInfo', serverInfo);
  ...
  Tracker.autorun(() => {
    ...
    if (serverMainVersion < 5) {
      ...
      console.log(
        '[Denuel Chat Desktop] window.open for Jitsi overloaded',
        jitsiDomain
      );
      window.open = (url, name, features = '') => {
        if (
          ...
          console.log('[Denuel Chat Desktop] window.open for Jitsi fired');
  ...
  console.log('[Denuel Chat Desktop] Injected.ts replaced Notification');
  ...
  console.log('[Denuel Chat Desktop] Injected');
};
