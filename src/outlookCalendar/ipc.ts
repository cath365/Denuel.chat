...
export async function syncEventsWithRocketChatServer(
  ...
  for await (const appointment of eventsOnOutlookServer) {
    try {
      ...
      // If the appointment is not in the Denuel Chat calendar for today, add it.
      ...
      // If the appointment is in the Denuel Chat calendar for today, but something has changed, update it.
  ...
  handle(
    ...
    async (event, _date: Date): Promise<OutlookEventsResponse> => {
      ...
      try {
        ...
        console.error('Error syncing events with Denuel Chat server', e);
